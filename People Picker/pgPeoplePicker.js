...
enyo.kind({
	...
	published: [
		...
		ppHeaderIconSytle: "padding-left: 10px; padding-top: 5px;",
		...
		ppFields: {}.
		ppFields.id: true,
		ppFields.displayName: true,
		ppFields.name: true,
		ppFields.nickname: true,
		ppFields.phoneNumbers: true,
		ppFields.emails: true,
		ppFields.addresses: true,
		ppFields.ims: true,
		ppFields.organizations: true,
		ppFields.birthday: true,
		ppFields.note: true,
		ppFields.photos: true,
		ppFields.categories: true,
		ppFields.urls: true,
	...
	components: [
		{name: "peoplePickerDialog", kind: "ModalDialog", layoutKind: "VFlexLayout", style: this.ppDialogStyle, components: [
			{kind: "HFlexBox", components: [
				{kind: "Image", src: this.ppHeaderIcon, style: this.ppHeaderIconStyle},
				{content: this.ppHeaderText, style: this.ppHeaderTextStyle},
				{kind: "Button", caption: this.ppHeaderButton, style: this.ppHeaderButtonStyle, onclick: "closePeoplePicker"},
			]},
			{kind: "HFlexBox", components: [
		...
		{     
			...
			else
			{
				...
				for(var key in ppFields)
				{
					if (ppFields.hasOwnProperty(key)) 
					{
						ppFieldsKeys.push(key);
						ppFieldsValues.push(ppFields[key]);
					}
				}
				...
				{
					navigator.contacts.find(this.ppFields, enyo.bind(this, this.peoplePickerSuccess), this.peoplePickerError, ppOptions);	
					success = true;
				}	
			}
		...
		peoplePickerSelection: function(recordIndex)
		{
			...
			if(record.addresses)
			{
				for(var aa = 0; aa < record.addresses.length; aa++)
				{
					...
					addy.pref = record.addressess[aa].pref;
					addy.type = record.addressess[aa].type;
					
					if(record.addressess[aa].streetAddress;) //NOTE: Android 1.x do not support this field.
					{
						addy.streetAddress = record.addressess[aa].streetAddress;
					}
					else
					{
						addy.streetAddress = "";
					...
					if(record.addressess[aa].locality) //NOTE: Android 1.x do not support this field.
					{
						addy.locality = record.addressess[aa].locality;	
					}
					else
					{
						addy.locality = "";	
					...
					if(record.addressess[aa].region)
					{
						addy.region = record.addressess[aa].region;
					}
					else
					{
					...
					if(record.addressess[aa].postalCode)
					{
						addy.postalCode = record.addressess[aa].postalCode;
					}
					else
					{
						addy.postalCode = "";
					...
					if(record.addressess[aa].country;)
					{
						addy.country = record.addressess[aa].country;
					}
					else
					{
						addy.country = "";
					...
					if(record.addressess[aa].formatted)
					{
						addy.formatted = record.addressess[aa].formatted;
					}
			...
			if(record.photos)
			{
				ppContacts.photos = record.photos;
			}

			if(record.categories) //NOTE: Android does not support this field
			{
				ppContacts.categories = record.categories;
			}
	...
	components: [
		{name: "ppList", kind: "VirtualRepeater", className: "", onSetupRow: "ppListSetupRow", components: [
			...
			{kind: "Item", className: "ppItem", tapHighlight: true, layoutKind: "HFlexLayout", onclick: "ppItemClick", components: [
				{name: "ppItemPhoto", className: "ppItem-photo"},
				{name: "ppItemName", className: "ppItem-name"}
			]}
		]}
	],
	...
	ppListSetupRow: function(inSender, inIndex) 
	{
		if(this.contacts)
		{
			...
			if (record) 
			{
				...
				if(this.contacts[inIndex].photo !== this.ppPhoto)
				{
